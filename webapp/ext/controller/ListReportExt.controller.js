        ...
        pDialog: null,
            ...
            openExcelUploadDialog: function(oEvent) {
                ...
                if (!this.pDialog) {
                    Fragment.load({
                        id: "excel_upload",
                        name: "fileupload.ext.fragment.ExcelUpload",
                        type: "XML",
                        controller: this
                    }).then((oDialog) => {
            ...
            callOdata: function (fnResolve, fnReject) {
                var oModel = this.getView().getModel();
                var payloadArray = []; // Array to store all payloads
            
                // Process each row from Excel
                this.excelSheetsData[0].forEach((value, index) => {
                    var payload = {  // Changed from 'let' to 'var' and moved inside loop
                        "Taxobject": value["Tax Object"] ? value["Tax Object"].toString() : "",
                        "GJAHR": value["Fiscal year"] ? value["Fiscal year"].toString() : "",
                        "FIACCRFA": value["FI Account"] ? value["FI Account"].toString() : "",
                        "TaxCountry": value["Tax Country"] ? value["Tax Country"].toString() : "",
                        "LegalEntid": value["Legal Entity"] ? value["Legal Entity"].toString() : "",
                        "Bukrs": value["Company Code"] ? value["Company Code"].toString() : "",
                        "InputParameter1": value["Input Parameter1"] ? value["Input Parameter1"].toString() : "",
                        "InputParameter2": value["Input Parameter2"] ? value["Input Parameter2"].toString() : "",
                        "Counter": value["Counter"] ? value["Counter"].toString() : "",
                        "Source": value["Source"] ? value["Source"].toString() : "",
                        "TradingPartner": value["Trading Partner"] ? value["Trading Partner"].toString() : "",
                        "Prefix": value["Prefix"] ? value["Prefix"].toString() : ""
